...
struct FathomCommand {
    ...
    private static func benchmark(
        ...
        if enforceReferenceGates {
            ...
            // The budget is a rate, not a stopwatch. Enumeration cost scales
            // with entries, not with gigabytes: a bare `find -xdev /` needs
            ...
            // on. 15,000 entries/s sits below the slowest of five whole-volume
            // runs, which spread 17,708 to 21,002 on machine load alone; below
            // 12,000 the engine has regressed rather than met a bigger disk.
            guard rate >= 12_000 else {
                throw CLIError.referenceGateFailed(
                    "scan rate was \(String(format: "%.0f", rate)) entries/second over \(traversal.entryCount) entries; 12000 blocks, 15000 is the target"
                )
            }
